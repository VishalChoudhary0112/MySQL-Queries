...
select bc.customer_id,customer_name,round(avg(balance_amount),2)
from bank_customer bc left join bank_account_details bad
...
(if(balance_amount=0,transaction_amount,balance_amount)),balance_amount)balance
...
select customer_id,bad.account_number,balance_amount,transaction_amount,transaction_date
from Bank_Account_Details bad join Bank_Account_Transaction bat
on bad.account_number=bat.account_number
where Transaction_Date between '2020-01-01' and '2020-04-01';
...
select account_number,account_type,event,customer_message
...
select bad.account_number,bad.account_type,balance_amount,bard.Account_Number,bard.Account_type
...
select account_number,account_type,sum(transaction_amount)
from Bank_Account_Details bad join Bank_Account_Transaction bat
using(account_number)
where account_type like '%credit%'
group by Account_type,account_number;
...
left join bank_account_transaction bt
on br.Account_Number=bt.account_number
where ba.account_type like '%Credit Card%'
group by ba.account_number, br.account_number;
