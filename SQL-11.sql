...
select product,round(avg(ifnull(estimated_sale_price,0)),2)
...
select branch,sum(revenue-cost),sum(Estimated_profit)
from Bank_branch_PL
group by branch
...
where Real_Profit > Total_Estimated_Profit;
...
select * from
(select Branch, Product, sum(Estimated_Profit)Total_Estimated_Profit, sum(revenue-cost*0.7)Cal_Profit
from Bank_Branch_PL
group by Branch, Product)t
where Total_Estimated_Profit<Cal_Profit;
