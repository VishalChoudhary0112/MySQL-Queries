...
select * from
(select sum(transaction_amount) Friday_transaction
from bank_account_transaction join bank_account_details 
using(account_number)
where Dayofweek(transaction_date)=6 and account_type like '%Credit card%')t1
cross join
(select sum(transaction_amount) Other_day_transaction
from bank_account_transaction join bank_account_details 
using(account_number)
...
select Account_Number, sum(if(transaction_date =any(select holiday from bank_holidays),0,transaction_amount)) as non_holidays,
 sum(if(transaction_date =any(select holiday from bank_holidays),transaction_amount,0)) holidays
from(select bac.Account_Number,Transaction_amount,Transaction_Date
 from bank_account_details bac join bank_account_transaction bat on bac.account_number=bat.Account_Number 
where Account_type like '%credit card%')t1 group by Account_Number;
...
from bank_account_details bd join bank_account_transaction bt
using(account_number)
