...
select name,platform,Year_of_Release,genre,row_number()over(partition by platform order by year_of_release desc)
from video_games_sales;
...
#Also arrange the result in the descending order by year of release.
  select name,platform,year_of_release,avg(global_sales)over(partition by year_of_release order by year_of_release desc)
  from video_games_sales;
   
-- ----------------------------------------------------------------------------------------------------
# 5. Display the name of the top 5 Games with highest Critic Score For Each Publisher. 

select name,platform,critic_rank from
(select name,platform,publisher,dense_rank()over(partition by publisher order by critic_score desc) as critic_rank
...
select name,lead(launch_date,2)over(order by launch_date)as launch_date
from web;

-- ----------------------------------------------------------------------------------------------------
# 7. Write a query that displays the statistics for website_id = 1 i.e. for each row, show the day, 
#the income and the income on the first day.
select day,income,first_value(income)over(rows between unbounded preceding and unbounded following)
from website_stats where website_id=1;

-----------------------------------------------------------------
# Dataset Used: play_store.csv 
-----------------------------------------------------------------
# 8. For each game, show its name, genre and date of release. In the next three columns, show RANK(),
# DENSE_RANK() and ROW_NUMBER() sorted by the date of release.
