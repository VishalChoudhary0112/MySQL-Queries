...
select sum(purchase_cost),round(avg(ifnull(estimated_sale_price,0)),2) 
...
select round(avg(ifnull(estimated_sale_price,0)),2) 
from bank_inventory_pricing;
...
select round(avg( if (purchase_cost=NULL,estimated_sale_price,purchase_cost ) ),2)
from bank_inventory_pricing;
...
select product,(max(ifnull(purchase_cost,0)) - min(ifnull(purchase_cost,0))) Difference
from bank_inventory_pricing
group by product;
...
group by product
having avg(ifnull(purchase_cost,0))>6000;
...
group by product
having avg(ifnull(purchase_cost,0))<sum(purchase_cost);
...
select product,round(avg(ifnull(estimated_sale_price,0)),2) from bank_inventory_pricing
...
select product,max(ifnull(estimated_sale_price,0))
from bank_inventory_pricing
where purchase_cost is not null and quantity>4
group by product;
...
select product,round(max(ifnull(estimated_sale_price,0)),2) from bank_inventory_pricing
group by product;
...
select product,avg(ifnull(estimated_sale_price,0))*1.15
from bank_inventory_pricing
group by product
having avg(ifnull(purchase_cost,0))>avg(ifnull(estimated_sale_price,0));
...
select avg(ifnull(purchase_cost,greatest(price,estimated_sale_price))) 
from bank_inventory_pricing
...
select product,avg(ifnull(price,0))
from bank_inventory_pricing
group by product
